...
import { onMounted, ref, computed, watch, nextTick, toRefs } from 'vue'
...
async function handleConnect(config: SshConnectionConfig, name: string): Promise<void> {
  console.log(`[app] handleConnect: ${name} (${config.username}@${config.host}:${config.port})`)
  addTab(config, name)
  try {
...
async function handleUpdate(
  ...
  console.log(`[app] update: ${name} (${config.username}@${config.host}:${config.port})`)
  try {
...
async function onConnected(tab: { id: string; connected: boolean; ftpConnected: boolean; loading: boolean; name: string; config: SshConnectionConfig; forkFrom?: string }): Promise<void> {
  console.log(`[app] tab connected`)
  tab.connected = true
  tab.loading = false
  if (state.settings.autoFtp && tab.name && tab.config && !tab.forkFrom) {
    try {
...
function getTabColor(tab: any) {
  ...
  if (ssh && ftp) return '#cdd6f4' // 全部连接 (Text)
  if (ssh && !ftp) return '#f9e2af' // 仅 SSH (Yellow)
  if (!ssh && ftp) return '#89b4fa' // 仅 FTP (Blue)
  return '#f38ba8' // 全部断开 (Red)
}

function getTabTooltip(tab: any) {
  return `SSH: ${tab.connected ? 'Connected' : 'Disconnected'} | FTP: ${tab.ftpConnected ? 'Connected' : 'Disconnected'}`
}
      ...
      <div class="titlebar-tabs frameless">
        <div class="tab-list">
          <div
            ...
            @dragend="onDragEnd"
          >
            <span class="tab-name" :style="{ color: getTabColor(tab) }" :title="getTabTooltip(tab)">{{ tab.name }}</span>
            <button
              v-if="!tab.connected"
              class="tab-refresh"
        ...
        <div class="titlebar-controls">
          <button class="titlebar-btn" @click="minimizeWindow" :title="$t('app.titlebar.minimize')">&#x2014;</button>
          <button class="titlebar-btn" @click="maximizeWindow" :title="$t('app.titlebar.maximize')">&#x25A1;</button>
          <button class="titlebar-btn titlebar-close" @click="closeWindow" :title="$t('app.titlebar.close')">&times;</button>
        </div>
      </div>
    </div>
      ...
      <div class="main-area">
        ...
        <div v-if="activeTabId && tabs.find(t => t.id === activeTabId)" class="subtab-bar">
          ...
          >
            <span
              class="status-dot"
              :class="(tabs.find(t => t.id === activeTabId)?.connected) ? 'connected' : 'disconnected'"
            ></span>
            {{ $t('app.subtab.ssh') }}
          </button>
          <button
          ...
          >
            <span
              class="status-dot"
              :class="(tabs.find(t => t.id === activeTabId)?.ftpConnected) ? 'connected' : 'disconnected'"
            ></span>
            {{ $t('app.subtab.ftp') }}
          </button>
        </div>
        <div class="content">
          <div v-if="tabs.length === 0" class="welcome">
            <div class="welcome-content">
              ...
              <button class="btn-primary" @click="openDialog">{{ $t('welcome.newConnection') }}</button>
            </div>
          </div>
          <div
            v-for="tab in tabs"
